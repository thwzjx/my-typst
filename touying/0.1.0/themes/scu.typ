...
#let register(
  ...
  self.scu-footer = self => {
    ...
    //   // rows: (1.5em, auto),
    //   align: right,
    //   image("../pic/四川大学.png", format: "png", width: 10%),
    // )
    // set align(bottom + right)
    // image("../pic/四川大学.png", format: "png", width: 10%)
    grid(
      columns: (33%,34%, 1fr,10%),
      rows: (auto, auto),
      // gutter: 3pt,
      // [],[],image("../pic/四川大学.png", width: 10%),
      cell(fill: self.colors.primary, utils.call-or-display(self, self.footer-a)),
      cell(fill: self.colors.secondary, utils.call-or-display(self, self.footer-b)),
      // cell(fill:self.colors.secondary-lighter,self.m-footer-right)
      cell(fill: self.colors.secondary-lighter, utils.call-or-display(self, footer-c)),cell(fill:self.colors.secondary-lighter,self.m-footer-right)
    )
  }
  let footer(self) = {
    set text(size: .8em)
