...
// sections
#let sections-state = state(
  "touying-sections-state",
  ((kind: "section", title: none, short-title: none, loc: none, count: 0, children: ()),),
)
...
#let touying-outline(enum-args: (:), padding: 0pt) = touying-final-sections(sections => {
  pad(
    ...
    enum(
      ..enum-args,
      ..sections.filter(section => section.loc != none).map(section => [#link(
          section.loc,
          section.title,
