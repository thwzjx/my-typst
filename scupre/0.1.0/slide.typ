...
#let _parse-touying-equation(self: none, need-cover: true, base: 1, index: 1, eqt) = {
  ...
  let result = ()
  let cover-arr = ()
  let children = it.split(regex("(#meanwhile;?)|(meanwhile)")).intersperse("touying-meanwhile").map(s => s.split(
    regex("(#pause;?)|(pause)"),
  ).intersperse("touying-pause")).flatten().map(s => s.split(
    regex("(\\\\\\s)|(\\\\\\n)"),
  ).intersperse("\\\n")).flatten().map(s => s.split(regex("&")).intersperse("&")).flatten()
...
#let touying-slide(
  ...
  if repeat == none {
    return {
      ...
      page(
        ..(self.page-args + (header: header, footer: footer)),
        setting-with-pad(page-preamble(1) + composer(..bodies)),
      )
    }
  }
  ...
  if self.handout {
    ...
    header = _update-states(1) + header
    page(..(self.page-args + (header: header, footer: footer)), setting-with-pad(page-preamble(1) + composer(..conts)))
  } else {
...
#let touying-slides(self: none, slide-level: 1, body) = {
  ...
  for child in children {
    i += 1
    if type(child) == content and child.func() == metadata and type(child.value) == dictionary and child.value.at(
      "kind",
      default: none,
    ) == "touying-slides-end" {
      is-end = true
      break
    } else if type(child) == content and child.func() == metadata and type(child.value) == dictionary and child.value.at(
      "kind",
      default: none,
      ...
      slide = utils.trim(slide)
      if (utils.heading-depth(child) == 1 and section != none) or (
        utils.heading-depth(child) == 2 and subsection != none
      ) or (utils.heading-depth(child) > slide-level and title != none) or slide != () {
        (self.methods.slide)(
          self: self,
          section: section,
          subsection: subsection,
